...
import { ChevronLeft, ChevronRight, Quote as QuoteIcon, Grid, Keyboard, MonitorPlay, Maximize2, Minimize2, ArrowRight } from 'lucide-react';
...
const SlideView: React.FC<{ slide: SlideData }> = ({ slide }) => {
  ...
  const renderContent = () => {
    switch (slide.type) {
      ...
      case 'author':
        return (
          <div className="h-full flex flex-col items-center justify-center text-center px-6 relative z-10 animate-in fade-in zoom-in duration-700">
            <div className="w-32 h-32 md:w-40 md:h-40 bg-gradient-to-br from-cyan-400 to-blue-600 rounded-full mb-8 flex items-center justify-center shadow-[0_0_40px_rgba(6,182,212,0.4)] border-4 border-slate-900 relative group">
              {/* Placeholder for Avatar if image available, else Initials */}
              <span className="text-4xl md:text-5xl font-bold text-white">
                {slide.author?.name.split(' ').map(n => n[0]).join('')}
              </span>
            ...
            <div className="flex flex-col items-center gap-2 mb-10">
              <span className="text-xl md:text-2xl text-cyan-400 font-medium">{slide.author?.role}</span>
              <span className="text-lg md:text-xl text-slate-400">{slide.author?.company}</span>
            </div>

            <a
              href={slide.author?.linkedin}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-3 px-8 py-4 bg-[#0077b5] hover:bg-[#006396] text-white rounded-full font-bold text-lg transition-all hover:scale-105 shadow-lg hover:shadow-blue-500/30 group"
            >
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col relative selection:bg-cyan-500/30 overflow-hidden font-sans">
      ...
      <main className="flex-1 relative overflow-hidden w-full">
        ...
        <div className={`absolute inset-0 w-full h-full z-20 ${animPhase === 'enter-start' ? 'slide-enter' :
